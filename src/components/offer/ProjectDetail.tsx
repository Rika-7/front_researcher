...
const ProjectField: React.FC<ProjectFieldProps> = ({
  ...
  return (
    <div className={`rounded min-h-[101px] ${className || ""}`}>
      <div className="flex-1 w-full bg-white rounded">
        <div className="flex flex-wrap flex-1 gap-1 items-start py-1 pl-4 rounded size-full">
          <div className="flex flex-col flex-1 shrink justify-center h-12 basis-0 min-w-60">
            <label className="flex-1 shrink self-stretch w-full text-base tracking-wide">
              {label}
            </label>
            <div className="flex-1 shrink self-stretch w-full text-2xl tracking-normal leading-7">
              {value}
            </div>
          </div>
...
export const ProjectDetail: React.FC = () => {
  return (
    <article className="flex flex-col grow py-12 pr-20 pl-4 w-full text-black whitespace-nowrap bg-white rounded-2xl shadow-[0px_0px_15px_rgba(0,0,0,0.03)]">
      <div className="flex flex-col mt-7">
        ...
        <ProjectField
          ...
          className="mt-24"
        />
        <ProjectField label="依頼日" value="2025年2月5日" className="mt-3.5" />
        <ProjectField
          label="返信希望日"
          value="2025年2月20日までに"
          className="mt-3.5"
